...
// Rota GET - retorna imagem como stream
router.get('/image', async (req, res) => {
    const { effect, text1 } = req.query;
    if (!effect || !text1) return res.status(400).send('effect e text1 são obrigatórios');

    const url = effectsMap[effect.toLowerCase()];
    if (!url) return res.status(404).send('Efeito não encontrado');

    try {
        const imageUrl = await ephoto(url, text1);
...
// Rota GET - página com todos os efeitos como links de teste
router.get('/', (req, res) => {
    let html = `<h1>Teste de efeitos ephoto360</h1><ul>`;
    for (const key of Object.keys(effectsMap)) {
        html += `<li><a href="/ephoto/image?effect=${key}&text1=Hello%20World" target="_blank">${key}</a></li>`;
    }
    html += `</ul>`;
    res.send(html);
});
